...
@views.route('/ingame/2/<lastcode>/<timeremaining>/count')
...
def chosen1(lastcode, onechoice):    #THIS PEICE OF CODE IS FOR PLAYER 1
    ...
    if choices1ids[-1] == lastcode:
        choice1 = choices1[-1]
    else:
        for items in choices1ids:
            if items == lastcode:
                position = choices1ids.index(items)
                choice1 =  choices1[position]
